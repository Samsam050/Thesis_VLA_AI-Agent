...
class FrankaRobot:
    def __init__(self) -> None:
        self._gripper_mode = GripperMode.IDLE
        self._last_target_width = None
    ...
    def update_gripper(self, command, velocity=True, blocking=False):
        ...
        WIDTH_EPS = 1e-3  # tolerance to avoid spam
        
        # if command > current_pos:
        #     self._gripper.grasp(speed=0.05, force=20, grasp_width=self._max_gripper_width * (1 - command), blocking=blocking)
        # else:
        #     print("releasing.")
        #     self._gripper.stop(blocking=blocking)
        #     self._gripper.goto(width=self._max_gripper_width * (1 - command), speed=0.05, force=0.0, blocking=blocking)
        
        # ---------------- GRASP ----------------
        if command > current_pos:
            if self._gripper_mode != GripperMode.GRASP:
                self._gripper.grasp(
                    speed=0.05,
                    force=50, 
                ...
                self._gripper_mode = GripperMode.GRASP
                self._last_target_width = target_width
            return
        """
        if command > current_pos:
            if self._gripper_mode != GripperMode.GRASP:
            ...
            """
    # ---------------- RELEASE ----------------
        if self._gripper_mode != GripperMode.RELEASE or (
            self._last_target_width is None or
            abs(target_width - self._last_target_width) > WIDTH_EPS
        ):
            if self._last_target_width is not None:
                print(f"The target width is {target_width:.4f}, the last_target_width is {self._last_target_width:.4f}")
            self._gripper.stop(blocking=blocking)
            self._gripper.goto(
                width=target_width,
                speed=0.05,
            ...
            self._gripper_mode = GripperMode.RELEASE
            self._last_target_width = target_width
    ...
    def create_action_dict(self, action, action_space, gripper_action_space=None, robot_state=None):
        ...
        if gripper_action_space == "velocity":
            ...
            # print(f"The received gripper command value is {action[-1]:.4f}")
            gripper_delta = self._ik_solver.gripper_velocity_to_delta(action[-1])
            # print(f"Gripper delta value is {gripper_delta:.4f}")
            
            gripper_position = robot_state["gripper_position"] + gripper_delta
            # print(f"The current gripper position is {robot_state['gripper_position']:.4f}, The target gripper position is {gripper_position:.4f}")
            ...
            print("target position", action_dict["gripper_position"], "current position", robot_state["gripper_position"])
