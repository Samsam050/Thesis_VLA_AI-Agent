...
enum class GripperMode { NONE, MOVE, GRASP, STOP };
...
void FrankaHandClient::applyGripperCommand(void) {
  is_moving_ = true;
  static GripperMode last_mode_ = GripperMode::NONE;

  if (gripper_cmd_.grasp()) {
    if (last_mode_ != GripperMode::GRASP) {
      gripper_->stop();
    }
    ...
    last_mode_ = GripperMode::GRASP;
  } else if (gripper_cmd_.stop()) {
    spdlog::info("stop the curtent behavior");
    gripper_->stop();
    last_mode_ = GripperMode::STOP;
  } else {
    spdlog::info("Moving to width {} at speed={}", gripper_cmd_.width(),
                 gripper_cmd_.speed());
    if (last_mode_ != GripperMode::MOVE) {
      gripper_->stop();
    }
    prev_cmd_successful_ =
        gripper_->move(gripper_cmd_.width(), gripper_cmd_.speed());
    last_mode_ = GripperMode::MOVE;
  }
