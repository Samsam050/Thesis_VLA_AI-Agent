...
from polymetis_pb2 import polymetis_pb2
from polymetis_pb2 import polymetis_pb2_grpc
import time
...
class GripperInterface:
    ...
    def __init__(self, ip_address: str = "localhost", port: int = 50052):
        ...
        self.last_mode = None  # "grasp" or "move"
    ...
    def goto(self, width: float, speed: float, force: float, blocking: bool = True):
        ...
        # print(f"== [2] The goto width is {width:.5f} ==")
        cmd = polymetis_pb2.GripperCommand(
            # width=width, speed=speed, force=force, grasp=False, stop=False
            width=0.08, speed=speed, grasp=False, stop=False
        )
        cmd.timestamp.GetCurrentTime()
    ...
    def grasp(
        ...
        self.last_force = force # Save the force memory

        print("Sending GRASP command...")
        cmd = polymetis_pb2.GripperCommand(
            width=0.0,
            speed=speed,
            force=force,
