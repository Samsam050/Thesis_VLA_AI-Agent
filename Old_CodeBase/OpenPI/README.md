...
Fine-Tuned models:
In this project π0.5-LIBERO	Inference is used. Pleae see LIBERO README. Checkpoint Path: gs://openpi-assets/checkpoints/pi05_libero
...
When one of these methods wants to be tested, please go to first:
...
If you are using the Ollama, please remember to start the model first.

Don't forget that you also need to start the VLA. Otherwise it will just sit and wait. 

To start VLA model:
docker compose -f scripts/docker/compose.yml up


To start simulator etc:
MUJOCO_GL=glx docker compose -f examples/libero/compose.yml run --build runtime
