...
class OakCamera:
    def __init__(self):
        ...
        cam.initialControl.setManualFocus(100) # 0..255

        # Request an output
        cam_out = cam.requestOutput((640, 480), type=dai.ImgFrame.Type.BGR888p)


        # Create output queue
        self.q_rgb = cam_out.createOutputQueue()
    ...
    def update(self):
        print("[OAK-D] Thread started — reading frames...")
        while self.running:
            frame_packet = self.q_rgb.tryGet()
            if frame_packet:
                self.frame = frame_packet.getCvFrame()
            else:
                time.sleep(0.001)
...
def main():
    ...
    while True:
        ...
        if frame_oak is None:
            frame_oak = np.zeros((480, 640, 3), dtype=np.uint8)
            cv2.putText(frame_oak, "WAITING FOR OAK...", (50, 240), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
            
        if frame_rs is None:
            frame_rs = np.zeros((480, 640, 3), dtype=np.uint8)
