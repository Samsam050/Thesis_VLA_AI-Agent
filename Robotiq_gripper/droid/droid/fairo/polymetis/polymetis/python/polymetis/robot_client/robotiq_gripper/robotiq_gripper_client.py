...
class RobotiqGripperClient:
    ...
    def __init__(self, server_ip, server_port, comport="/dev/ttyUSB0", hz=60):
        ...
        self.gripper.sendCommand()
        time.sleep(2)
        ready = self.gripper.is_ready()
        # while not ready:
        #     ready = self.gripper.is_ready()
        #     status = self.gripper.getStatus()
        #     print("wait to ready")
        #     print("The value of act is", self.gripper.gSTA)
        #     time.sleep(0.5)
        sent = self.gripper.sendCommand()
        # if not sent:
        #     print("[debug]: send command fials]")
        status = self.gripper.getStatus()

        print(f"is_ready={ready}, sendCommand={sent}, getStatus={status}")
        # ready = False
        # while not ready:
        #     self.gripper.getStatus()
        #     ready = ((self.gripper.gSTA == 3 and self.gripper.gACT == 1) or (self.gripper.gSTA == 0 and self.gripper.gACT == 1))
        #     time.sleep(0.5)
        #     print("Wait to be ready, self.gripper.gSTA is", self.gripper.gSTA)
